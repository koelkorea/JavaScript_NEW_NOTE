...
- JS에서 [배열]과 연관된 특수사항  
                           ...
                           e. heap영역에 [배열]의 요소들이 저장되는 구조는 ArrayList와 같은 주소가 물리적으로 연속되지 않고, LinkedList처럼 메모리주소값으로 연결된 구조
                              (= 알고리즘 짜면 많이 불리해짐)
- [배열]과 연관된 메서드
                           A. {다른 참조 자료형} -> [배열]로 변환하는 메서드

                              a. Array.from(참조 자료형)
                                  : 참조 자료형에 해당하는 {유사 배열(Array-like) 객체}나 {이터러블(iterable) 객체}를 [배열]로 변환하여 반환해주는 {Array 객체} 그 자체의 메서드
                              ...
                              b. '문자열'.split('기준 문자')
                                  : '문자열'을 '기준 문자'를 기준으로 잘라서, 그렇게 잘려진 것들을 [배열]의 요소로 담아 구성된 [배열을 반환함] 
 
                                    ex) '1,2,3,4,5'라는 문자열을 ','을 기준으로 잘라서 그렇게 나눠진 요소들로 [배열]을 구성하여 반환
                                    
                                        '1,2,3,4,5'.split(',');
                                          -> [ 1, 2, 3, 4, 5 ]
  
                           B. 참조자료형의 메모리 위치에 있는 [어떤 배열]의 요소를 CRUD 기능으로 조작하기 위한 [배열]의 메서드

                              a. [대상 배열].push(...[배열] 요소로 넣을 값)
                                  : 매개변수(parameter)에 할당된 인자(augument)들을 [대상 배열] 그 자체의 맨 '끝 부분'에 추가하는 메서드
      
                                    ex) [23, 24]로 저장된 [배열] 끝에 "Hello", "나는"이라는 문자열을 [배열]의 마지막 요소로 1번에 추가
            
                                        [23, 24].push("Hello", "나는");
                                         -> [23, 24, "Hello", "나는"]

                              b. [대상 배열].unshift(...[배열] 요소로 넣을 값)
                                  : 매개변수(parameter)에 할당된 인자(augument)들을 [대상 배열] 그 자체의 맨 '처음 부분'에 추가하는 메서드

                                    ex) [23, 24]로 저장된 [배열] 끝에 "Hello", "나는"이라는 문자열을 [배열]의 첫 요소로 1번에 추가
            
                                        [23, 24].unshift("Hello", "나는");
                                         -> ["Hello", "나는", 23, 24]

                              c. [대상 배열].pop()
                                  : [대상 배열] 그 자체의 맨 '끝' 인덱스의 요소 1개를 '삭제'하는 메서드

                                    ex) [23, 24]로 저장된 [배열] 맨 '끝' 인덱스의 요소 1개를 '삭제'
            
                                        [23, 24].pop();
                                         -> [23]

                              d. [대상 배열].shift()
                                  : [대상 배열] 그 자체의 맨 '처음' 인덱스의 요소 1개를 '삭제'하는 메서드

                                    ex) [23, 24]로 저장된 [배열] 맨 '처음' 인덱스의 요소 1개를 '삭제'
            
                                        [23, 24].shift();
                                         -> [24]

                              e. [대상 배열].splice(시작 인덱스, 지울 요소 갯수, ...추가할 요소값);
                                  : [대상 배열]의 시작 인덱스 위치부터 오른쪽을 향해 지정개수까지 요소들을 제거하고, 추가할 요소값에 있는 요소들을 순차적으로 끼워넣어서 [대상 배열]의 데이터를 수정함
                                      -> (중요) splice 이용시 주의사항
- 2번째 parameter에 할당된 '지울 요소 갯수'는 생략이 가능하나, 생략하면 '시작 인덱스'를 기준으로 오른쪽의 모든 [배열] 요소들이 삭제됨
- 3 ~ n번째 parameter에 할당된 ...추가할 요소값은 생략이 가능함

                                             ex) [23, 24, 25, 26]로 저장된 [배열]의 1번 인덱스를 시작으로 2개의 요소를 삭제하고, 30, 31, 32, 33 4개의 요소를 [배열][1]의 요소를 시작으로 [배열][4]에 해당하는 위치까지 순차적으로 끼워넣음
                     
                                                 [23, 24, 25, 26].splice(1, 2, 30, 31, 32, 33);
                                                  -> [23, 30, 31, 32, 33, 26]

                              f. [대상 배열].concat([합칠 배열]);
                                  : '[대상 배열][시작 인덱스] ~ [대상 배열][끝 인덱스]'의 [배열] 내용을 복사해서, [합칠 배열]의 뒷쪽에 push 메서드와 마찬가지로 합쳐서 붙이는 메서드

                                    ex) [23, 24, 25, 26, 27]로 저장된 [배열] 끝에 ["나는", "you", 0, "지팡이"]를 추가함
            
                                        [23, 24, 25, 26, 27].concat(["나는", "you", 0, "지팡이"]);
                                         -> [23, 24, 25, 26, 27, 0, "지팡이"]

                              g. [대상 배열].fill(값, 시작 인덱스, 끝 인덱스);
                                  : '[대상 배열][시작 인덱스] ~ [대상 배열][끝 인덱스]'에 특정 값들을 각 index의 요소로 대입해주는 메서드

                                    ex) [23, 24, 25, 26, 27]로 저장된 [배열]의 index가 2~5에 해당하는 값으로 'a'를 넣어서 수정
            
                                        [23, 24, 25, 26, 27].fill('a' 2, 5);
                                         -> [23, 24, 'a', 'a', 'a', 'a']

                              h. [대상 배열].copyWithin([소스 배열], 시작 인덱스, 끝 인덱스);
                                  : '[소스 배열][시작 인덱스] ~ [소스 배열][끝 인덱스]'의 [배열] 내용을 복사해서, [대상 배열][시작 인덱스] ~ [대상 배열][끝 인덱스]에 완벽히 그 내용을 덮어씌우는 메서드
                                    (= [대상 배열]의 겹치는 위치의 내용은 전부 덮어쓰기 당함)
                                        -> (중요) copyWithin 이용시 주의사항
- '[대상 배열].length > [소스 배열].length' 면 덮어쓰기에서 벗어난 [대상 배열]의 데이터는 남아있음

                                    ex) [23, 24, 25, 26, 27]로 저장된 [배열] 끝에 ["나는", "you", 0, "지팡이"]의 2번 ~ 3번 인덱스만 [0, "지팡이"] 복사해서, [23, 24, 25, 26, 27]의 뒤에 추가되며, 애초에 [대상 배열]의 index길이는 5라서 [소스 배열]이 닿지 않는 [23, 24, 25, 26, 27][4]의 값은 유지됨
            
                                        [23, 24, 25, 26, 27].copyWithin(["나는", "you", 0, "지팡이"], 2, 3);
                                         -> [23, 24, 0, "지팡이", 27]

                           C. 참조자료형의 메모리 위치에 있는 [어떤 배열]의 정렬 순서를 영구히 변경하기 위한 [배열]의 메서드

                              a. [대상 배열].sort(무명콜백함수(비교1, 비교2){ return 숫자})
...
- sort 메서드를 사용하여, 정렬하면 원본 [배열]의 정렬까지 변하니, 그에 대비해야 함
                           ...
                           D. 무명콜백함수의 내용부를 [어떤 배열] 내부의 요소들을 활용하여 순차적으로 실행시키는 [배열]의 메서드들

                              a. [대상 배열].foreach(무명콜백함수(개별요소, 인덱스, 배열자체){ 내용부 })
                              ...
                              b. [대상 배열].reduce, reduceRight(무명콜백함수(현재 누적값, 개별요소){ return 수식 }, 초기값)
...
- (중요) reduce <-> reduceRight 의 차이
                           ...
                           E. 특정 값이 [대상 배열]의 요소로서 존재하는지를 단순 검색하기 위한 메서드들

                              a. [대상 배열].indexOf(값)
                                  : 파라미터로 놓은 값이 [대상 배열] 요소에 있는지를 index가 0 ~ n 순서 기준으로 검색해서, 있으면 해당 인덱스 중 가장 작은 값에 해당하는 인뎅스값 출력,  없으면 -1 출력

                                    ex) [7,3,5,2,7,40] 중 7에 해당하는 요소가 1번째에 바로 발견되기에 해당 인덱스에 해당하는 0 반환
                                    
                                        var 어레이 = [7,3,5,2,40,7];
                                        var 위치값 = 어레이.indexOf(7);  

                                    ex) [7,3,5,2,7,34] 중 8에 해당하는 요소가 없기에 해당 인덱스에 해당하는 -1 반환
                                    
                                        var 어레이 = [7,3,5,2,40,7];
                                        var 위치값 = 어레이.indexOf(8);

                              b. [대상 배열].lastIndexOf(값)
                                  : 파라미터로 놓은 값이 [대상 배열] 요소에 있는지를 index가 n~0 순서 기준으로 검색해서, 가장 뒷쪽에 있는 요소의 인덱스를 출력, 없으면 -1 출력

                                    ex) [7,3,5,2,7,40] 중 7에 해당하는 요소가 6번째에 바로 발견되기에 해당 인덱스에 해당하는 5 반환
                                    
                                        var 어레이 = [7,3,5,2,40,7];
                                        var 위치값 = 어레이.lastIndexOf(7);  

                                    ex) [7,3,5,2,7,34] 중 8에 해당하는 요소가 없기에 해당 인덱스에 해당하는 -1 반환
                                    
                                        var 어레이 = [7,3,5,2,40,7];
                                        var 위치값 = 어레이.lastIndexOf(8);

                              c. [대상 배열].includes(값)
                                  : 파라미터로 놓은 값에 해당하는 [대상 배열]이 요소에 있으면 true, 없으면 false

                                    ex) [7,3,5,2,7,40] 중 7에 해당하는 요소가 바로 발견되기에 true 반환
                                    
                                        var 어레이 = [7,3,5,2,40,7];
                                        var 위치값 = 어레이.includes(7);  

                                    ex) [7,3,5,2,7,34] 중 8에 해당하는 요소가 없기에 false 반환
                                    
                                        var 어레이 = [7,3,5,2,40,7];
                                        var 위치값 = 어레이.includes(8);

                           F. [어떤 배열]의 요소들 중 '무명콜백함수'의 반환되는 수식 조건을 충족시키는 요소가 있는지를 고차원적으로 검색하기 위한 메서드들

                              a. [대상 배열].find(무명콜백함수(개별요소, 인덱스, 배열자체{ return 수식 })
                              ...
                              b. [대상 배열].some(무명콜백함수(개별요소, 인덱스, 배열자체{ return 수식 })
                              ...
                              c. [대상 배열].every(무명콜백함수(개별요소, 인덱스, 배열자체{ return 수식 })
                           ...
                           G. [대상 배열] 내부의 각 요소를 순차적으로 무명콜백함수의 조건식과 비교한 뒤, [원본 배열]을 훼손하지 않는 이를 충족하는 요소들로 구성된 [새로운 배열]을 구성하여 반환하는 '함수형 프로그래밍 패러다임'을 따르는 [배열]의 메서드

                              a. [대상 배열].filter(무명콜백함수(개별요소, 인덱스, 배열자체{ return 조건식 })
...
- 원하는 조건식을 썼으면, 그 앞에 return을 반드시 적어줘야 filter 메서드는 작동하게됨
                              ...
                              b. [대상 배열].map(무명콜백함수(개별요소, 인덱스, 배열자체{ return 수식 })
             ...
             # {object(객체)} 종류
                 1) { literal type object(리터럴 형식 객체) }
                      : 사용자가 직접 {객체}의 내용부를 작성하여, 특정 변수나 상수에 저장한 {객체}
                         ...
                         -> (참고) 리터럴(Literal)
                              : 사전적으로는 '문자 그대로'라는 뜻 = 프로그래밍 언어에서는 특정한 자료형의 값을 직접 정해진 표기법으로 표현하는 방식 (= 각 자료형마다의 작성법 = 리터럴)
  ...
  9. JSON 전역 객체
      : JS에서 JavaScript Object Notation(JSON)을 분석하거나 값을 JSON으로 변환하는 메서드를 가지는 전역객체  
       ...
       @ JavaScript Object Notation(JSON)
          : Javascript {객체} 문법과 유사한 '문자열' 기반의 데이터 포맷... 데이터를 직렬화(object -> string 변환) 하고 전송하는데 쓰이는 '범용적' 데이터 포맷
            (= 프로그래밍 언어나 플랫폼에 상관없이 독립적으로 사용이 가능함)
               -> JSON의 특성
                  1) JSON은 순수히 키-값(key-value)으로 묶인 데이터 포맷 (= 멤버변수 O , 메서드 X, Symbol X)
       ...
       @ JSON 메서드
         1) JSON.stringify(array/object 변수)
            : paramter의 array/object -> JSON 변환하여 문자열로서 저장 가능하게 하는 JSON 객체의 내부 함수
               -> 선택 사항으로 특정 속성만 포함하거나, 사용자 정의 방식으로 속성을 대체 가능한 오버로딩 함수가 존재 

             # JSON.stringify()의 오버로딩 함수들

               a. JSON.stringify(array/object 변수, [변화시키고 싶은 프로퍼티명들로만 구성된 배열]) 
                   : {객체}나 [배열] 안의 원하는 프로퍼티만 뽑아서 { }안의 기존 json 형식으로 값을 변환하여 반환하는 JSON 객체의 내부 함수

                     ex) {objExample 객체}의 프로퍼티 중 key가 'name', 'date'에 해당하는 녀석만 추려서 {objExample 객체}를 Json 형식으로 변환하여 반환하여 'json'이라는 변수에 저장함
                     
                         json = JSON.stringify(objExample, ['name', 'date']);

              b. JSON.stringify(array/object 변수,  무명콜백함수( key, value ) { 변환식 return key, value를 포함 } );
                   : 무명콜백 함수를 통해, {객체}나 [배열] 안의 key, value를 입맛에 맞게 바꾼 결과를 json 형식으로 변환하여 반환하는 JSON 객체의 내부 함수

                     ex) {objExample 객체}의 프로퍼티 중 key가 'name'에 해당하는 녀석이 있다면, 그 프로퍼티의 값을 'ellie'라고 바꾼뒤, 해당 {objExample 객체}를 Json 형식으로  변환 후 'json'이라는 변수에 반환하여 저장함
                     
                         json = JSON.stringify(objExample, (key, value) => {
         ...
         2) JSON.parse(JSON 형식 구문)
            : paramter의 JSON 형식 문자열을 JSON으로서 구문 분석하여, JSON구문 -> array/object나 숫자 문자 반환
               -> 선택적으로 분석 결과의 값과 속성을 변환해 반환
                   -> BUT! {object}의 함수는 복구할 수 없기에 parameter에 대입해봐야 의미가 없음

             # JSON.parse()의 오버로딩 함수들

              b. JSON.parse(JSON 형식 구문,  무명콜백함수( key, value ) { 변환식 return key, value를 포함 } );
                   : 무명콜백 함수를 통해, JSON 형식 구문 안의 key, value를 입맛에 맞게 바꾼 결과를 {객체} 자료형으로 변환하여 반환하는 JSON 객체의 내부 함수

                     ex) 'objExample json객체'의 프로퍼티 중 key가 'name'에 해당하는 녀석이 있다면, 그 프로퍼티의 값을 'apple'라고 바꾼뒤, 해당 'objExample json객체'를 {객체}로 변환 후 'obj'이라는 변수에 반환하여 저장함
                     
                         obj = JSON.parse(objExample, (key, value) => {
                         
                           console.log(`key: ${key}, value: ${value}`);
                           return key === 'apple'? 'ellie' : value; 
                         });
